...
elseif RequiredScript == "lib/managers/hud/hudchallengenotification" then
    if not (MUIMenu and MUIMenu:ClassEnabled("MUIHint") and MUIMenu:ClassEnabled("MUIPresentItem") and MUIMenu._data.mui_challanges) then return; end
    ...
    function HudChallengeNotification:init(title, text, icon, rewards, queue)
        ...
        local hint = managers.hud._hud_hint;
        if hint then
            local full_text = title .. "\n" .. text;
            hint:show({text = full_text});
        end
        self:close();
    end
end
	
--------------------- OLD_SETUP_CHALLENGE_NOTIFICATIONS --------
-- Keeping this in case I wish to revert it back from using the hint panel
-- elseif RequiredScript == "lib/managers/hud/hudchallengenotification" then
