...
elseif RequiredScript == "lib/managers/hud/hudchallengenotification" then
	...
	function HudChallengeNotification:init(title, text, icon, rewards, queue)
		...
		local total_w = noti_text:w();

		local row_h = noti_text:h();

		if icon_texture then
			local icon_h = 50 * self._scale;
			row_h = math.max(noti_text:h(), icon_h);
			...
			noti_text:set_left(icon:right() + 5);
			noti_text:set_center_y(row_h / 2);

			total_w = icon:w() + 5 + noti_text:w();
		else
			noti_text:set_left(0);
		end
